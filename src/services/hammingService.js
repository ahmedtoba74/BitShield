...
function calculateParity(d1, d2, d3, d4) {
    // معادلات الـ Parity (Even Parity)
    // p1 covers d1, d2, d4
...
exports.encode = (binaryString) => {
    let encodedData = "";
    // إضافة أصفار في الآخر لو الطول مش بيقبل القسمة على 4
...
exports.decode = (noisyBinaryString) => {
    ...
    let uncorrectedBlocks = 0; // للحالات اللي فيها أكثر من خطأ (نظرياً في (7,4) صعب اكتشافه بدقة بس هنعد التعديلات)
    ...
    for (let i = 0; i < noisyBinaryString.length; i += 7) {
        ...
        // لو الـ Syndromes كلها 0 يبقى مفيش خطأ
        if (s1 === 0 && s2 === 0 && s3 === 0) {
            correctedBinary += `${d1}${d2}${d3}${d4}`;
        } else {
            // فيه خطأ، نحدد مكانه ونصلحه
            // Mapping syndrome (s1,s2,s3) binary to error position logic
            // هنا للتبسيط هنشوف مين المعادلة اللي باظت ونستنتج
    ...
    // ملحوظة: بنرجع الكود مصحح 100% بالنسبة لقدرة هامينج
    return { 
        correctedBinary, 
        correctionReport: { correctedErrors: totalErrorsCorrected, efficiency: "Calculated based on total noise" } 
    };
};
